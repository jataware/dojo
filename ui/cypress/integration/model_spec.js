...
describe('Creating a model', () => {
  ...
  // it('Revisits an existing model', () => {
    ...
    cy.get('.xterm-helper-textarea')
      ...
      .then(() => {
        cy.frameLoaded({ url: '/api/shorthand' }, { timeout: 60000 }).then(($iframe) => {
          ...
          $iframe[0].contentWindow.postMessage(JSON.stringify({
            ...
            content_id: '/home/clouseau/file.txt',
          }), '*');
    ...
    cy.get('.xterm-helper-textarea')
      .type('tag data.csv {enter}', { timeout: 300000 }).then(() => {
        ...
        // overview screen
        cy.frameLoaded(
          { url: '/api/spacetag/overview' }, { timeout: 300000 }
        ).then(() => {
