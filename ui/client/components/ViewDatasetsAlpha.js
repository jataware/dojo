...
function ViewDatasets() {
  ...
  const handleRowExpanderClick = (cell) => {
    console.log('row expander clicked', cell);

    // TODO set expanded row?
  };
  ...
  const columns = [
    {
      ...
      headerName: "",
      renderCell: (cell) => (
        <div className={classes.rowExpander}>
          <IconButton onClick={() => handleRowExpanderClick(cell)}>
            <ChevronRightIcon />
          </IconButton>
        </div>
      ),
