...
const LaunchContainer = async (worker, imageName, dockerImage, modelId) => {
  const resp = await fetch(`/api/clouseau/docker/${worker}/launch`, {
    method: 'POST',
    headers: {
...
const CheckGitUrl = async (url) => {
  const resp = await fetch('/api/clouseau/cors/test', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
const PingContainer = async (worker) => {
  const resp = await fetch(`/api/clouseau/container/${worker}/ops/`);
  if (!resp.ok) {
...
const Provision = async (worker, containerId, cmd) => {
  ...
  const resp = await fetch(`/api/clouseau/docker/${worker}/exec/${containerId}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
